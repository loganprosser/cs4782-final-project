...
def main() -> None:
    ...
    if args.experiment in {"main", "both"}:
        print("\n[main] Checking results/mnist")
        mnist_dir = root_dir / "results" / "mnist"
        ...
        regression_metrics = root_dir / "results" / "regression" / "regression_metrics.json"
        ...
        if args.generate and main_ok:
            summary_dir = "results/summary_best_val" if args.selection_mode == "best_val" else "results/summary_last_epoch"
            run_command(
                [python_bin, "code/summarize_results.py", "--selection-mode", args.selection_mode, "--summary-dir", summary_dir],
                root_dir,
                args.dry_run,
            )
    ...
    if args.include_shift_check:
        shift_metrics = root_dir / "results" / "uncertainty_shift" / "uncertainty_shift_metrics.json"
