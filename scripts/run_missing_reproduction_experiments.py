...
BBB_CODE = REPO_ROOT / "bayes-by-backprop-reimplementation" / "code"
sys.path.insert(0, str(BBB_CODE))

from losses import bayesian_classification_loss  # noqa: E402
from models import BayesianMLP, DropoutMLP, StandardMLP  # noqa: E402
...
def ensure_dirs() -> None:
    for path in [
        REPO_ROOT / "reproduction",
        REPO_ROOT / "reproduction" / "logs",
        REPO_ROOT / "reproduction" / "checkpoints",
        REPO_ROOT / "reproduction" / "figures",
    ]:
...
def write_warning(message: str) -> None:
    log_path = REPO_ROOT / "reproduction" / "logs" / "run_warnings.log"
...
def completed(method: str, seed: int, epochs: int) -> bool:
    final_path = REPO_ROOT / "reproduction" / "final_metrics.csv"
...
def make_loaders(batch_size: int, seed: int, quick: bool) -> tuple[DataLoader, DataLoader, DataLoader, int]:
    data_dir = REPO_ROOT / "bayes-by-backprop-reimplementation" / "data"
...
def append_log(method: str, seed: int, message: str) -> None:
    path = REPO_ROOT / "reproduction" / "logs" / f"{method}_seed{seed}.log"
...
def run_method(args: argparse.Namespace, method: str, seed: int, device: torch.device) -> None:
    ...
    checkpoint_rel = Path("reproduction") / "checkpoints" / f"{method}_seed{seed}.pt"
