...
checkpoint_exists() {
  local run_name="$1"
  [[ -f "${ROOT_DIR}/results/bayes_by_backprop/mnist/checkpoints/${run_name}_best.pt" ]]
}
...
if [[ "${INCLUDE_TRUST_RUNS}" == "true" ]]; then
  run_cmd "${PYTHON_BIN}" code/experiments/benchmark_uncertainty_shift.py \
    --selection-mode "${SELECTION_MODE}" \
    --summary-dir results/bayes_by_backprop/uncertainty_shift \
    --output-dir results/bayes_by_backprop/uncertainty_shift \
...
echo "Summary: ${ROOT_DIR}/results/bayes_by_backprop/uncertainty_shift/uncertainty_shift_summary.md"
