...
MNIST_RUN_COMMANDS = {
    "standard_trust_none": 'run_cmd "${PYTHON_BIN}" code/experiments/train_mnist.py --model standard --bayesian-dropout 0.0 --update-trust-mode none --epochs "${MNIST_EPOCHS}" --batch-size "${MNIST_BATCH_SIZE}" --seed "${SEED}"',
    "dropout_trust_none": 'run_cmd "${PYTHON_BIN}" code/experiments/train_mnist.py --model dropout --bayesian-dropout 0.0 --update-trust-mode none --epochs "${MNIST_EPOCHS}" --batch-size "${MNIST_BATCH_SIZE}" --seed "${SEED}"',
    "bayesian_trust_none": 'run_cmd "${PYTHON_BIN}" code/experiments/train_mnist.py --model bayesian --bayesian-dropout 0.0 --update-trust-mode none --epochs "${MNIST_EPOCHS}" --batch-size "${MNIST_BATCH_SIZE}" --seed "${SEED}"',
    "bayesian_dropout_0p1_trust_none": 'run_cmd "${PYTHON_BIN}" code/experiments/train_mnist.py --model bayesian --bayesian-dropout 0.1 --update-trust-mode none --epochs "${MNIST_EPOCHS}" --batch-size "${MNIST_BATCH_SIZE}" --seed "${SEED}"',
}
...
def materialize_aliases(root_dir: Path) -> list[str]:
    ...
    for alias, source in MNIST_ALIASES.items():
        if alias not in mnist_metrics and source in mnist_metrics:
            ...
            changed.append(f"results/bayes_by_backprop/mnist/test_accuracy.json:{alias} <- {source}")
    if changed:
    ...
    for alias, source in REGRESSION_ALIAS.items():
        if alias not in regression_metrics and source in regression_metrics:
            ...
            changed.append(f"results/bayes_by_backprop/regression/regression_metrics.json:{alias} <- {source}")
    if regression_metrics and any(item.startswith("results/bayes_by_backprop/regression") for item in changed):
...
def write_run_script(root_dir: Path, output_path: Path, missing_mnist: list[str], regression_missing: bool) -> None:
    ...
    lines.extend(
        [
            ...
            'run_cmd "${PYTHON_BIN}" code/evaluation/check_and_generate_plots.py --experiment both --selection-mode best_val --generate',
            "",
            'echo',
