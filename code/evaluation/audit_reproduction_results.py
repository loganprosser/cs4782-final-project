...
REPRO = REPO_ROOT / "results" / "reproduction"
BBB = REPO_ROOT / "results" / "bayes_by_backprop"
...
def read_accuracy_table() -> list[dict[str, str]]:
    table = BBB / "mnist" / "accuracy_table.csv"
    if not table.exists():
        return []
...
def checkpoint_status() -> list[str]:
    old_ckpts = find_files(["*.pt", "*.pth", "*.ckpt"], [BBB / "mnist" / "checkpoints"])
    repro_ckpts = find_files(["*.pt", "*.pth", "*.ckpt"], [REPRO / "checkpoints"])
    lines = [
        f"- Old MNIST checkpoints found: `{len(old_ckpts)}` in `results/bayes_by_backprop/mnist/checkpoints/`.",
        f"- Reproduction checkpoints found: `{len(repro_ckpts)}` in `results/reproduction/checkpoints/`.",
    ]
...
def dependency_notes() -> list[str]:
    ...
    notes.append("- The checked project virtual environment is `.venv/bin/python`; use it if the system `python3` lacks ML dependencies.")
...
def build_markdown() -> str:
    script_files = find_files(["*.py", "*.sh"], [BBB, REPO_ROOT / "code", REPO_ROOT / "code" / "kalman_trust" / "kalmanalgo", REPO_ROOT / "code" / "kalman_trust" / "sample_trust"])
    data_files = find_files(["*.csv", "*.json", "*.log", "*.txt"], [BBB, REPO_ROOT / "results" / "archived_sample_trust_runs", REPRO])
    plot_files = find_files(["*.png", "*.svg", "*.pdf"], [BBB, REPO_ROOT / "poster", REPO_ROOT / "report", REPRO / "figures"])
    md_files = find_files(["*.md"], [REPO_ROOT])
    rows = read_accuracy_table()
    aggregate = inspect_json(BBB / "mnist" / "test_accuracy.json")
...
## Scope
This audit covers the repository rooted at `{REPO_ROOT}` with special attention to `code/bayes_by_backprop/`, which contains the Bayes by Backprop implementation.
...
- `code/experiments/train_mnist.py` trains `standard`, `dropout`, and `bayesian` models. `bayesian` with `--bayesian-dropout 0.1` implements Bayes by Backprop + Dropout.
- `code/experiments/benchmark_mnist_compute.py` benchmarks per-step compute cost.
- `code/evaluation/summarize_results.py` builds the old result summaries.
- `code/experiments/run_missing_reproduction_experiments.py` is the new reproduction runner that writes the requested CSVs, logs, and checkpoints under `results/reproduction/`.
- `code/evaluation/make_reproduction_plots.py` is the new plot/report generator for the requested figures and markdown.

## Existing Model Implementations
- Vanilla/standard MLP: `StandardMLP` in `code/bayes_by_backprop/models.py`.
- Dropout MLP: `DropoutMLP` in `code/bayes_by_backprop/models.py`.
- Bayes by Backprop MLP: `BayesianMLP` using `BayesianLinear` in `code/bayes_by_backprop/models.py` and `code/bayes_by_backprop/bayesian_layers.py`.
...
- Core old MNIST files include `results/bayes_by_backprop/mnist/accuracy_table.csv` and `results/bayes_by_backprop/mnist/test_accuracy.json`.
- Existing history status: {history_note}

## Existing Plots
- Found `{len(plot_files)}` plot/PDF artifacts outside virtual environments.
- Old MNIST/summary plots are mostly under `results/bayes_by_backprop/mnist/` and `results/bayes_by_backprop/summary*`.
- New reproduction figures are generated under `results/reproduction/figures/`.

## Existing Result Summaries And README Files
- Found `{len(md_files)}` markdown files outside virtual environments.
- Notable existing summaries: `results/bayes_by_backprop/summary_best_val/experiment_summary.md`, `results/bayes_by_backprop/summary_best_val/compute_summary.md`, and `README.md`.
...
- Organized poster/report-ready PNG figures under a top-level `results/reproduction/` directory.
...
- `results/reproduction/training_curves.csv`: `python code/experiments/run_missing_reproduction_experiments.py --epochs 20 --batch-size 128 --lr 1e-3 --seeds 0 --device auto`
- `results/reproduction/final_metrics.csv`: same runner.
- `results/reproduction/resource_metrics.csv`: same runner.
- `results/reproduction/runtime_summary.csv`: same runner.
- `results/reproduction/memory_summary.csv`: same runner.
- `results/reproduction/paper_reproduction_summary.csv`: `python code/evaluation/make_reproduction_plots.py`
- `results/reproduction/figures/*.png`: `python code/evaluation/make_reproduction_plots.py`
- `results/reproduction/reproduction_summary.md`: `python code/evaluation/make_reproduction_plots.py`
- `results/reproduction/figure_captions.md`: `python code/evaluation/make_reproduction_plots.py`

## Exact Commands
Use the project virtual environment when running from this checkout:

```bash
.venv/bin/python code/evaluation/audit_reproduction_results.py
.venv/bin/python code/experiments/run_missing_reproduction_experiments.py --epochs 20 --batch-size 128 --lr 1e-3 --seeds 0 --device auto
.venv/bin/python code/evaluation/make_reproduction_plots.py
```

For a quick sanity check:

```bash
.venv/bin/python code/experiments/run_missing_reproduction_experiments.py --quick --epochs 2 --batch-size 128 --lr 1e-3 --seeds 0 --device auto --force
```

For a fuller multi-seed run:

```bash
.venv/bin/python code/experiments/run_missing_reproduction_experiments.py --epochs 100 --batch-size 128 --lr 1e-3 --seeds 0 1 2 --device auto
```
...
## Weight Histograms
Weight histograms can be generated from reproduction checkpoints in `results/reproduction/checkpoints/` and from compatible old checkpoints. For Bayesian models, the plotting script uses posterior mean weights (`weight_mu`) as a stable approximation to the trained weight distribution. The optional plot is skipped with a warning if checkpoint formats are incompatible.
